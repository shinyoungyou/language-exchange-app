...
export default observer(function MessageListItem({message}: Props) {
  ...
  return (
      <div className='messageListItem'>
        <Image as={Link} to={`/members/${author}`} width='60rem' circular src={user?.username === message.senderUsername ?  message.recipientPhotoUrl:  message.senderPhotoUrl || require("@/assets/img/user.png")} />
