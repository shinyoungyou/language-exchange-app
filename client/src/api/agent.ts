...
const Members = {
    ...
    listFollowings: (username: string, predicate: string) => requests.get<Member[]>(`/follow/${username}?predicate=${predicate}`),
    follow: (username: string) => requests.put<void>(`/follow/${username}`, {}),
    unfollow: (username: string) => requests.del<void>(`/follow/${username}`),
    ...
    // report: (activity: ActivityFormValues) => requests.put<void>(`/activities/${activity.id}`, activity),
}
